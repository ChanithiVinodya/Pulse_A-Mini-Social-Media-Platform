...
const setupEventListeners = () => {
  ...
  document.getElementById('start-chat-btn')?.addEventListener('click', () => {
     document.getElementById('sidebar-search-btn')?.click() || document.getElementById('bottom-search-btn')?.click();
  });
...
const openConversation = async (userId) => {
  ...
  try {
    ...
    // Set header info
    const user = conversations.find(c => c.user.id === userId)?.user || await fetchUserInfo(userId);
...
const renderMessages = (messages) => {
  ...
  if (messages.length === 0) {
    messagesEl.innerHTML = '<div class="chat-start-msg">This is the start of your conversation.</div>';
    return;
  }
